...
public abstract class PacketQueue<T>
{
     ...
     */
    public synchronized void add(byte[] buf, int off, int len)
    {
     ...
     * packet.
     */
    public synchronized void add(byte[] buf, int off, int len, Object context)
    {
        if (copy)
        {
     ...
     */
    public synchronized void add(T pkt)
    {
        if (copy)
        {
     ...
     * Blocks until there is a packet in the queue.
     ...
     * the queue is non-empty. If the queue is empty, returns null without
     * blocking.
    ...
    /**
     * Returns the packet ({@code T}) at the head of this queue without removing
     * it from the queue. If the queue is empty, returns null without blocking.
     *
     * Note that while this method will not remove a packet from the queue,
     * there is no guarantee that subsequent calls to {@link #peek()},
     * {@link #poll()} or {@link #get()} will return the same packet. This is
     * because packet will be dropped from the queue while adding to it, if its
     * capacity is reached.
     *
     * @return the packet at the head of this queue, or null if the queue is
     * empty. Does not remove a packet from the queue.
     */
    /* We should probably not expose this because of the note above.
    public T peek()
    {
        if (handler != null)
        {
